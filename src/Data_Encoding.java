...
public class Data_Encoding {

	public static long getSizeOfUnCompressedIndex(
			Map<String, Build_Index.DictionaryEntry> uncompressedIndex) {
		long lenght = 0;
		for (String term : uncompressedIndex.keySet()) {
			Build_Index.DictionaryEntry entry = uncompressedIndex.get(term);
			lenght += entry.term.length() + 2 * Integer.SIZE + 8; // Length(term)
																	// +
																	// size(df)
																	// +
																	// size(tf)
																	// + size of
																	// pointer
			lenght += entry.postingList.size() * 2 * Integer.SIZE; // list size
																	// *
																	// (size(docId)
																	// +
																	// size(tf))
	...
	public static Map<String, DictionaryEntry> createCompressedIndex(
		...
		for (String term : uncompressedIndex.keySet()) {
			...
			int prevDocId = 0;
			for (Build_Index.PostingEntry postingEntry : entry.postingList) {
	...
	public static long getSizeOfCompressedIndex(
		...
		for (String term : compressedIndex.keySet()) {
			...
			lenght += entry.term.length() + entry.docFrequency.length
					+ entry.termFrequency.length + 8; // Length(term) + size(df)
														// + size(tf) + size of
														// pointer
	...
	public static long getSizeOfCompressedPostingList(
		...
		for (PostingEntry postingEntry : postingList) {
			lenght += postingEntry.docID.length + postingEntry.frequency.length; // list
																					// size
																					// *
																					// (size(docId)
																					// +
